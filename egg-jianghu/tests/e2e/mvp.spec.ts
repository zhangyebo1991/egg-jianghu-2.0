...
test('返回总览隐藏二级导航且通关基础后可穿越下一位面', async ({ page }) => {
  ...
  expect(await page.evaluate(() => window.__EGG_JIANGHU__.getSelection()))
    .toEqual({ worldId: 'world_01', difficulty: 1, stage: 10, mode: 'guard' })
  await openPanel(page, 'idle')
  await page.getByTestId('world-world_02').click()
  ...
  await expect(page.getByTestId('faction-quest-board')).toHaveAttribute('data-world-id', 'world_02')
...
test(`兑换页仅在正式势力解锁后按原版贡献价格出售转职书 ${width}`, async ({ page }, testInfo) => {
  ...
  expect(after.contribution.world_01).toBe(0)
})
}
...
test('位面十五格悬榜锁定已接任务并刷新未接任务', async ({ page }) => {
  await page.evaluate(() => {
    window.__EGG_JIANGHU__.unlockFaction('tieyi_school')
    window.__EGG_JIANGHU__.grantContribution('world_01', 400)
  ...
  const after = await page.evaluate(() => window.__EGG_JIANGHU__.getState().factionBoards.world_01.slots.map((slot) => slot?.id ?? null))
...
test('兑换页聚合位面声望、贡献兑换与招募', async ({ page }, testInfo) => {
  ...
  expect(await page.evaluate(() => window.__EGG_JIANGHU__.getState().contribution.world_01)).toBe(78_516)
  ...
  expect(await page.evaluate(() => window.__EGG_JIANGHU__.getState().contribution.world_01)).toBe(73_828)
