...
test('启动后可见挂机战斗并能访问四个核心 Tab', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('heading', { name: '青石古道' })).toBeVisible()
  ...
  await expect(page.getByRole('heading', { name: '古道破关' })).toBeVisible()
...
test('挂机所得可用于招募同门并激活羁绊', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /队伍/ }).click()
  await page.locator('select[data-action="party-slot"]').nth(1).selectOption('yan_qiusheng')
  await expect(page.getByText('丐帮共鸣')).toBeVisible()
  await expect(page.getByText(/全队攻势 \+12%/)).toBeVisible()
})

test('挑战获胜后进度写入 localStorage 并能在刷新后恢复', async ({ page }) => {
  await page.getByRole('button', { name: /战斗/ }).click()
  await page.getByRole('button', { name: '挑战第 1 关' }).click()
  await page.evaluate(() => window.__EGG_JIANGHU__.advanceCombat(80))
  await expect(page.locator('.battle-result.victory')).toBeVisible()
  expect(await page.evaluate(() => window.__EGG_JIANGHU__.getState().clearedStage)).toBe(1)

  await page.reload()
  expect(await page.evaluate(() => window.__EGG_JIANGHU__.getState().clearedStage)).toBe(1)
  await expect(page.getByRole('button', { name: /战斗/ })).toContainText('已破 1 关')
})
