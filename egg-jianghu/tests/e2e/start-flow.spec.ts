...
test('version 17 旧档不可继续且新建和删除 version 19 存档都不覆盖旧档', async ({ page }) => {
  await page.evaluate(() => window.localStorage.setItem('egg-jianghu-2-save-v17', 'version-17-save'))
  await page.reload()

  await expect(page.getByRole('button', { name: '继续游戏' })).toBeDisabled()
  await expect(page.getByRole('alert')).toHaveText('检测到旧版存档；当前版本需要新建游戏，旧档保留但不可继续')
  ...
  expect(await page.evaluate(() => window.localStorage.getItem('egg-jianghu-2-save-v19'))).toBeNull()
  expect(await page.evaluate(() => window.localStorage.getItem('egg-jianghu-2-save-v17'))).toBe('version-17-save')
})
...
test('覆盖前重新检查存档并在存档变化后要求再次确认', async ({ page }) => {
  ...
  await page.evaluate(() => window.localStorage.setItem('egg-jianghu-2-save-v19', 'marker-1'))
  await page.getByLabel('玩家姓名').press('Enter')

  await expect(page.getByTestId('overwrite-confirmation')).toBeVisible()
  expect(await page.evaluate(() => window.localStorage.getItem('egg-jianghu-2-save-v19'))).toBe('marker-1')

  await page.evaluate(() => window.localStorage.setItem('egg-jianghu-2-save-v19', 'marker-2'))
  await page.getByRole('button', { name: '确认覆盖并开始' }).click()
  await expect(page.getByTestId('overwrite-confirmation')).toBeVisible()
  await expect(page.getByRole('status')).toHaveText('存档已发生变化，请重新确认覆盖')
  expect(await page.evaluate(() => window.localStorage.getItem('egg-jianghu-2-save-v19'))).toBe('marker-2')
...
test('多标签页同步存档状态且旧会话不能覆盖外部删档或新档', async ({ page, context }) => {
  ...
  expect(await page.evaluate(() => {
    const raw = window.localStorage.getItem('egg-jianghu-2-save-v19')
...
test('损坏的玩家姓名存档拒绝继续且不会被当前会话覆盖', async ({ page }) => {
  ...
  expect(await page.evaluate(() => window.localStorage.getItem('egg-jianghu-2-save-v19'))).toBe(corruptedSave)
