...
import { initializeQuestBoard, QUEST_SLOT_COUNT } from './quests'
...
const agentState = (): GameStateV10 => {
  ...
  state.factionBoards[WORLD_ID] = {
    ...
    slots: [boardQuest('q0'), ...Array.from({ length: QUEST_SLOT_COUNT - 1 }, () => null)],
  }
  return state
}
...
describe('位面代理人任务自动化', () => {
  it('未任命或开关关闭时不接受也不交付', () => {
    ...
    expect(idle.factionBoards[WORLD_ID].slots[0]?.acceptedRecordId).toBe(0)
  ...
  it('玩家不在任何位面时只交付、不接受', () => {
    ...
    state.factionBoards[WORLD_ID].slots[1] = { ...boardQuest('q1'), acceptedRecordId: 1 }

    const result = runFactionAgentAutomation(state, null)

    expect(result).toEqual({ accepted: 0, completed: 1 })
    expect(state.factionBoards[WORLD_ID].slots[0]?.acceptedRecordId).toBe(0)
  })
  ...
  it('进度达标即自动交付，并在同一 tick 内补接一轮', () => {
    ...
    state.acceptedFactionQuests = { 1: { ...acceptedKillQuest(1, 1), progress: 2 } }
    state.factionBoards[WORLD_ID].slots[1] = { ...boardQuest('q1'), acceptedRecordId: 1 }
    const contributionBefore = state.contribution[WORLD_ID] ?? 0
    ...
    expect(state.contribution[WORLD_ID]).toBeGreaterThan(contributionBefore)
    expect(state.factionBoards[WORLD_ID].slots[1]?.acceptedRecordId).toBe(-1)
  })
  ...
  it('并发已接任务达到原版上限 12 后停止接受', () => {
    const state = agentState()
    state.factionBoards[WORLD_ID].slots = Array.from(
      { length: QUEST_SLOT_COUNT },
      (_, index) => boardQuest(`q${index}`),
    )
    ...
    expect(runFactionAgentAutomation(state, WORLD_ID).accepted).toBe(0)
    expect(state.factionBoards[WORLD_ID].slots.every((slot) => slot?.acceptedRecordId === 0)).toBe(true)
  })
})
...
describe('位面代理人筛选矩阵', () => {
  ...
  it('排除品质列后该品质不再被自动接受，其他品质仍放行', () => {
    ...
    state.factionBoards[WORLD_ID].slots[0] = boardQuest('q0', 1, 2)
