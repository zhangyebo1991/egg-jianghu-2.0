...
describe('邀请与武功修习', () => {
  it('从明确名单直接邀请侠客，不返回随机结果', () => {
    ...
    const result = recruitFromTavern(state, 'hero_shen_yanqiu')

    expect(result).toEqual({ ok: true, heroId: 'hero_shen_yanqiu', spent: 280 })
    expect(state.heroes.hero_shen_yanqiu.recruited).toBe(true)
    expect(state.worldCurrency.world_01).toBe(720)
  })

  it('A1 Lv.20 只解锁同线 B1，遗忘返还分账投入的 80%', () => {
    const state = seededState({ world_01: 1000 }, { qingfeng_hall: 1000 })
    const hero = state.heroes.hero_shen_yanqiu

    expect(learnFactionMartial(state, 'hero_shen_yanqiu', 'qingfeng_hall_b1').ok).toBe(false)
    hero.learnedMartials.qingfeng_hall_a1 = learnedAt(20, {
      contribution: { qingfeng_hall: 500 },
      worldCurrency: {},
    })
    expect(learnFactionMartial(state, 'hero_shen_yanqiu', 'qingfeng_hall_b1').ok).toBe(true)
    expect(learnFactionMartial(state, 'hero_shen_yanqiu', 'qingfeng_hall_b2').ok).toBe(false)

    const refund = forgetMartial(state, 'hero_shen_yanqiu', 'qingfeng_hall_a1')
  ...
  it('每名侠客只能主修一门心法且不占四个主动槽', () => {
    ...
    expect(equipHeartMethod(state, 'hero_shen_yanqiu', 'qingfeng_hall_heart_01').ok).toBe(true)

    expect(state.heroes.hero_shen_yanqiu.heartMethodId).toBe('qingfeng_hall_heart_01')
    expect(state.heroes.hero_shen_yanqiu.equippedMartialIds).toEqual([null, null, null, null])
  })
  ...
  it('生成 30 势力共 240 门主动武功且第 21 门被拒绝', () => {
    ...
    expect(learnFactionMartial(state, 'hero_shen_yanqiu', 'qingfeng_hall_a1').ok).toBe(false)
  })
  ...
  it('四个主动槽按指定位置装备且禁止重复', () => {
    ...
    expect(equipMartial(state, 'hero_shen_yanqiu', 'qingfeng_hall_a1', 2).ok).toBe(true)
    expect(hero.equippedMartialIds).toEqual([null, null, 'qingfeng_hall_a1', null])
    expect(equipMartial(state, 'hero_shen_yanqiu', 'qingfeng_hall_a1', 0).ok).toBe(false)
  })
})
