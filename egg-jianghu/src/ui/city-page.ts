...
import { careerIconAsset, martialIconAsset } from './career-icon-assets'
...
export const renderCityPage = (view: CityPageViewModel): string => {
  ...
  <div class="city-columns">
    <section class="tavern panel"><header><small>酒馆名录</small><strong>明码直接邀请</strong></header>${view.tavernHeroes.map((hero) => `<article data-testid="tavern-${hero.id}"><img class="city-portrait" src="${escapeHtml(heroPortraitAsset(hero.id).url)}" alt="" aria-hidden="true" draggable="false"><span data-rarity="${escapeHtml(hero.grade)}">${escapeHtml(hero.grade)}</span><div><strong>${escapeHtml(hero.name)}</strong><small>本卷货币 ${hero.cost}</small></div><button type="button" data-action="tavern-recruit" data-hero-id="${hero.id}" ${hero.recruited ? 'disabled' : ''}>${hero.recruited ? '已加入' : '直接邀请'}</button></article>`).join('')}</section>
    <section class="academy panel"><header><small>城市武馆</small><strong>通用武功</strong></header>${view.martials.map((martial) => `<article data-rarity="${escapeHtml(martial.rarity)}"><img class="city-martial-icon" src="${escapeHtml(martialIconAsset(martial.id))}" alt="" aria-hidden="true" draggable="false"><div><strong>${escapeHtml(martial.name)}</strong><small>${escapeHtml(martial.rarity)} · 本卷货币 ${martial.cost}</small></div><button type="button" data-action="city-martial-learn" data-hero-id="${view.selectedHeroId ?? ''}" data-martial-id="${martial.id}" ${martial.learned ? 'disabled' : ''}>${martial.learned ? '已学习' : '学习'}</button></article>`).join('')}</section>
    <section class="token-shop panel"><header><small>转职信物</small><strong>高阶信物随江湖卷开放</strong></header>${view.careerTokens.map((token) => `<article><img class="city-token-icon" src="${escapeHtml(careerIconAsset(token.id.replace(/^token_/, '')))}" alt="" aria-hidden="true" draggable="false"><div><strong>${escapeHtml(token.name)}</strong><small>${escapeHtml(token.tier)} · 本卷货币 ${token.cost}</small></div><button type="button" data-action="career-buy-token" data-world-id="${view.worldId}" data-token-id="${token.id}" ${token.owned ? 'disabled' : ''}>${token.owned ? '已持有' : '购取'}</button></article>`).join('')}</section>
  </div>
</section>`
}
