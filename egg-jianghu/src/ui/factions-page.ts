...
export const renderFactionsPage = (view: FactionsPageViewModel): string => {
  ...
  return `<section class="factions-layout" data-testid="factions-page">
    ...
    <div class="faction-content">
      ...
      ${view.factionHero ? `<section class="faction-invite panel"><div><small>势力侠客</small><strong data-rarity="${escapeHtml(view.factionHero.grade)}">${escapeHtml(view.factionHero.name)}</strong><span>贡献 ${view.factionHero.cost}</span></div><button type="button" data-action="faction-recruit" data-faction-id="${view.selectedFactionId}" data-hero-id="${view.factionHero.id}" ${view.factionHero.recruited ? 'disabled' : ''}>${view.factionHero.recruited ? '已加入' : '直接邀请'}</button></section>` : ''}
    </div>
  </section>`
}
