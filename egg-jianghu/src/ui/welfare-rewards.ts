...
export const renderWelfareRewards = (rewards: readonly WelfareReward[]): string => `<dialog class="welfare-dialog" aria-labelledby="welfare-reward-title" data-testid="welfare-rewards">
  ...
  <ul>${rewards.map(reward => `<li><img src="${escapeHtml(heroAppearanceAsset(reward.id))}" alt="${escapeHtml(reward.name)}"><span class="welfare-hero-label">福利侠客</span><h3>${escapeHtml(reward.name)} × ${reward.amount}</h3><p>已加入侠客名录，可前往阵容上阵</p></li>`).join('')}</ul>
